/******************************
  Program "./protocols/n_godsont.m" compiled by "Caching Murphi Release 5.5.0"
...
#define PROTOCOL_NAME "./protocols/n_godsont"
...
class mu_1_TYPE_NODE: public mu__byte
{
 public:
  ...
  mu_1_TYPE_NODE (const char *name, int os): mu__byte(1, 2, 2, name, os) {};
  mu_1_TYPE_NODE (void): mu__byte(1, 2, 2) {};
  mu_1_TYPE_NODE (int val): mu__byte(1, 2, 2, "Parameter or function result.", 0)
  {
    operator=(val);
  };
...
/*** Variable declaration ***/
mu_1_TYPE_NODE mu_curNode("curNode",224);

/*** Variable declaration ***/
mu_1_TYPE_CACHE mu_curCache("curCache",232);

/*** Variable declaration ***/
mu_1_TYPE_ADDR mu_curMemory("curMemory",240);

/*** Variable declaration ***/
mu_1_TYPE_DATA mu_curData("curData",248);

/*** Variable declaration ***/
mu_1_TYPE_LOCK mu_curLock("curLock",256);

/*** Variable declaration ***/
mu_1_REPLACE_STAGE mu_replace("replace",264);

/*** Variable declaration ***/
mu_1_REPLACE_RULE mu_repRule("repRule",272);
...
bool Condition(unsigned r)
{
  ...
  if (r>=8 && r<=23) return R2.Condition(r-8);
  if (r>=24 && r<=55) return R3.Condition(r-24);
  if (r>=56 && r<=71) return R4.Condition(r-56);
  if (r>=72 && r<=79) return R5.Condition(r-72);
  if (r>=80 && r<=95) return R6.Condition(r-80);
  if (r>=96 && r<=96) return R7.Condition(r-96);
  if (r>=97 && r<=104) return R8.Condition(r-97);
  if (r>=105 && r<=120) return R9.Condition(r-105);
  if (r>=121 && r<=121) return R10.Condition(r-121);
  if (r>=122 && r<=137) return R11.Condition(r-122);
  if (r>=138 && r<=153) return R12.Condition(r-138);
  if (r>=154 && r<=161) return R13.Condition(r-154);
  if (r>=162 && r<=177) return R14.Condition(r-162);
  if (r>=178 && r<=185) return R15.Condition(r-178);
  if (r>=186 && r<=189) return R16.Condition(r-186);
  if (r>=190 && r<=190) return R17.Condition(r-190);
  if (r>=191 && r<=198) return R18.Condition(r-191);
  if (r>=199 && r<=206) return R19.Condition(r-199);
  if (r>=207 && r<=210) return R20.Condition(r-207);
  if (r>=211 && r<=214) return R21.Condition(r-211);
  if (r>=215 && r<=216) return R22.Condition(r-215);
  if (r>=217 && r<=218) return R23.Condition(r-217);
  if (r>=219 && r<=222) return R24.Condition(r-219);
  if (r>=223 && r<=224) return R25.Condition(r-223);
...
void Code(unsigned r)
{
  ...
  if (r>=8 && r<=23) { R2.Code(r-8); return; } 
  if (r>=24 && r<=55) { R3.Code(r-24); return; } 
  if (r>=56 && r<=71) { R4.Code(r-56); return; } 
  if (r>=72 && r<=79) { R5.Code(r-72); return; } 
  if (r>=80 && r<=95) { R6.Code(r-80); return; } 
  if (r>=96 && r<=96) { R7.Code(r-96); return; } 
  if (r>=97 && r<=104) { R8.Code(r-97); return; } 
  if (r>=105 && r<=120) { R9.Code(r-105); return; } 
  if (r>=121 && r<=121) { R10.Code(r-121); return; } 
  if (r>=122 && r<=137) { R11.Code(r-122); return; } 
  if (r>=138 && r<=153) { R12.Code(r-138); return; } 
  if (r>=154 && r<=161) { R13.Code(r-154); return; } 
  if (r>=162 && r<=177) { R14.Code(r-162); return; } 
  if (r>=178 && r<=185) { R15.Code(r-178); return; } 
  if (r>=186 && r<=189) { R16.Code(r-186); return; } 
  if (r>=190 && r<=190) { R17.Code(r-190); return; } 
  if (r>=191 && r<=198) { R18.Code(r-191); return; } 
  if (r>=199 && r<=206) { R19.Code(r-199); return; } 
  if (r>=207 && r<=210) { R20.Code(r-207); return; } 
  if (r>=211 && r<=214) { R21.Code(r-211); return; } 
  if (r>=215 && r<=216) { R22.Code(r-215); return; } 
  if (r>=217 && r<=218) { R23.Code(r-217); return; } 
  if (r>=219 && r<=222) { R24.Code(r-219); return; } 
  if (r>=223 && r<=224) { R25.Code(r-223); return; } 
}
int Priority(unsigned short r)
{
  if (r<=3) { return R0.Priority(); } 
  if (r>=4 && r<=7) { return R1.Priority(); } 
  if (r>=8 && r<=23) { return R2.Priority(); } 
  if (r>=24 && r<=55) { return R3.Priority(); } 
  if (r>=56 && r<=71) { return R4.Priority(); } 
  if (r>=72 && r<=79) { return R5.Priority(); } 
  if (r>=80 && r<=95) { return R6.Priority(); } 
  if (r>=96 && r<=96) { return R7.Priority(); } 
  if (r>=97 && r<=104) { return R8.Priority(); } 
  if (r>=105 && r<=120) { return R9.Priority(); } 
  if (r>=121 && r<=121) { return R10.Priority(); } 
  if (r>=122 && r<=137) { return R11.Priority(); } 
  if (r>=138 && r<=153) { return R12.Priority(); } 
  if (r>=154 && r<=161) { return R13.Priority(); } 
  if (r>=162 && r<=177) { return R14.Priority(); } 
  if (r>=178 && r<=185) { return R15.Priority(); } 
  if (r>=186 && r<=189) { return R16.Priority(); } 
  if (r>=190 && r<=190) { return R17.Priority(); } 
  if (r>=191 && r<=198) { return R18.Priority(); } 
  if (r>=199 && r<=206) { return R19.Priority(); } 
  if (r>=207 && r<=210) { return R20.Priority(); } 
  if (r>=211 && r<=214) { return R21.Priority(); } 
  if (r>=215 && r<=216) { return R22.Priority(); } 
  if (r>=217 && r<=218) { return R23.Priority(); } 
  if (r>=219 && r<=222) { return R24.Priority(); } 
  if (r>=223 && r<=224) { return R25.Priority(); } 
return 0;}
char * Name(unsigned r)
{
  if (r<=3) return R0.Name(r-0);
  if (r>=4 && r<=7) return R1.Name(r-4);
  if (r>=8 && r<=23) return R2.Name(r-8);
  if (r>=24 && r<=55) return R3.Name(r-24);
  if (r>=56 && r<=71) return R4.Name(r-56);
  if (r>=72 && r<=79) return R5.Name(r-72);
  if (r>=80 && r<=95) return R6.Name(r-80);
  if (r>=96 && r<=96) return R7.Name(r-96);
  if (r>=97 && r<=104) return R8.Name(r-97);
  if (r>=105 && r<=120) return R9.Name(r-105);
  if (r>=121 && r<=121) return R10.Name(r-121);
  if (r>=122 && r<=137) return R11.Name(r-122);
  if (r>=138 && r<=153) return R12.Name(r-138);
  if (r>=154 && r<=161) return R13.Name(r-154);
  if (r>=162 && r<=177) return R14.Name(r-162);
  if (r>=178 && r<=185) return R15.Name(r-178);
  if (r>=186 && r<=189) return R16.Name(r-186);
  if (r>=190 && r<=190) return R17.Name(r-190);
  if (r>=191 && r<=198) return R18.Name(r-191);
  if (r>=199 && r<=206) return R19.Name(r-199);
  if (r>=207 && r<=210) return R20.Name(r-207);
  if (r>=211 && r<=214) return R21.Name(r-211);
  if (r>=215 && r<=216) return R22.Name(r-215);
  if (r>=217 && r<=218) return R23.Name(r-217);
  if (r>=219 && r<=222) return R24.Name(r-219);
  if (r>=223 && r<=224) return R25.Name(r-223);
  return NULL;
}
};
const unsigned numrules = 225;

/********************
  parameter
 ********************/
#define RULES_IN_WORLD 225
...
bool mu__condition_226() // Condition for Rule "Coherence, a:1, j:2, i:1"
{
  return mu__invariant_214( 1, 2, 1 );
}

bool mu__condition_227() // Condition for Rule "Coherence, a:1, j:2, i:2"
{
  return mu__invariant_214( 1, 2, 2 );
}

bool mu__condition_230() // Condition for Rule "Coherence, a:2, j:1, i:1"
{
  return mu__invariant_214( 2, 1, 1 );
}

bool mu__condition_231() // Condition for Rule "Coherence, a:2, j:1, i:2"
{
  return mu__invariant_214( 2, 1, 2 );
}

bool mu__condition_233() // Condition for Rule "Coherence, a:2, j:2, i:1"
{
  return mu__invariant_214( 2, 2, 1 );
}

bool mu__condition_234() // Condition for Rule "Coherence, a:2, j:2, i:2"
{
...
int mu__invariant_235( const mu_1_TYPE_NODE &mu_i) // Invariant "DeadlockFree"
{
bool mu__boolexpr236;
bool mu__boolexpr237;
  if (!((mu_replace) == (mu_NON))) mu__boolexpr237 = FALSE ;
  else {
  mu__boolexpr237 = (mu_node[mu_i].mu_hasLock) ; 
}
...
bool mu__boolexpr240;
  if (!(mu_lock[mu_l].mu_beUsed)) mu__boolexpr240 = FALSE ;
  else {
  mu__boolexpr240 = ((mu_lock[mu_l].mu_owner) == (mu_i)) ; 
}
if ( (mu__boolexpr240) )
  { mu__quant239 = TRUE; break; }
};
};
  if (!(mu__quant239)) mu__boolexpr238 = FALSE ;
  else {
bool mu__quant241; 
mu__quant241 = TRUE;
{
...
bool mu__boolexpr245;
  if ((mu_m) == (mu_n)) mu__boolexpr245 = TRUE ;
  else {
  mu__boolexpr245 = (!(mu_lock[mu_m].mu_beUsed)) ; 
}
  if (mu__boolexpr245) mu__boolexpr244 = TRUE ;
  else {
  mu__boolexpr244 = (!(mu_lock[mu_n].mu_beUsed)) ; 
}
  if (mu__boolexpr244) mu__boolexpr243 = TRUE ;
  else {
  mu__boolexpr243 = ((mu_lock[mu_m].mu_owner) != (mu_i)) ; 
}
  if (mu__boolexpr243) mu__boolexpr242 = TRUE ;
  else {
  mu__boolexpr242 = ((mu_lock[mu_n].mu_owner) != (mu_i)) ; 
}
if ( !(mu__boolexpr242) )
  ...
  mu__boolexpr238 = (mu__quant241) ; 
}
  mu__boolexpr236 = (mu__boolexpr238) ; 
}
return mu__boolexpr236;
};

bool mu__condition_247() // Condition for Rule "DeadlockFree, i:1"
{
  return mu__invariant_235( 1 );
}

bool mu__condition_248() // Condition for Rule "DeadlockFree, i:2"
{
  return mu__invariant_235( 2 );
}

/**** end rule declaration ****/

const rulerec invariants[] = {
{"DeadlockFree, i:2", &mu__condition_248, NULL, },
{"DeadlockFree, i:1", &mu__condition_247, NULL, },
{"Coherence, a:2, j:2, i:2", &mu__condition_234, NULL, },
{"Coherence, a:2, j:2, i:1", &mu__condition_233, NULL, },
{"Coherence, a:2, j:1, i:2", &mu__condition_231, NULL, },
{"Coherence, a:2, j:1, i:1", &mu__condition_230, NULL, },
{"Coherence, a:1, j:2, i:2", &mu__condition_227, NULL, },
{"Coherence, a:1, j:2, i:1", &mu__condition_226, NULL, },
