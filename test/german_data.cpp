/******************************
  ...
  Murphi Last Compiled date: "Oct  8 2022"
 ******************************/
...
class mu_1_DATA: public mu__long
{
 public:
  inline int operator=(int val) { return mu__long::operator=(val); };
  inline int operator=(const mu_1_DATA& val) { return mu__long::operator=((int) val); };
  mu_1_DATA (const char *name, int os): mu__long(1, 10000, 14, name, os) {};
  mu_1_DATA (void): mu__long(1, 10000, 14) {};
  mu_1_DATA (int val): mu__long(1, 10000, 14, "Parameter or function result.", 0)
  {
    operator=(val);
  };
  char * Name() { return tsprintf("%d",value()); };
...
/*** Variable declaration ***/
mu_1__type_5 mu_ShrSet("ShrSet",336);

/*** Variable declaration ***/
mu_0_boolean mu_ExGntd("ExGntd",352);

/*** Variable declaration ***/
mu_1_MSG_CMD mu_CurCmd("CurCmd",360);

/*** Variable declaration ***/
mu_1_NODE mu_CurPtr("CurPtr",368);

/*** Variable declaration ***/
mu_1_DATA mu_MemData("MemData",376);

/*** Variable declaration ***/
mu_1_DATA mu_AuxData("AuxData",408);
...
void SetNextEnabledRule(unsigned & what_rule)
{
  ...
  if (what_rule>=22 && what_rule<20022)
    { R11.NextRule(what_rule);
      if (what_rule<20022) return; }
}
bool Condition(unsigned r)
{
  category = CONDITION;
  ...
  if (r>=22 && r<=20021) return R11.Condition(r-22);
...
void Code(unsigned r)
{
  ...
  if (r>=22 && r<=20021) { R11.Code(r-22); return; } 
}
int Priority(unsigned short r)
{
  ...
  if (r>=22 && r<=20021) { return R11.Priority(); } 
return 0;}
char * Name(unsigned r)
{
  ...
  if (r>=22 && r<=20021) return R11.Name(r-22);
  return NULL;
}
};
const unsigned numrules = 20022;

/********************
  parameter
 ********************/
#define RULES_IN_WORLD 20022
...
void Code(unsigned short r)
{
  if (r<=9999) { S0.Code(r-0); return; }
}
void Goal_value() {  // makun: set goalstate

  // r-value is int type: mu_i.goal_value(1);
  mu_ExGntd.value(true);
  // mu_Cache[1].mu_State.value(mu_S);
  // mu_Cache[2].mu_State.value(mu_S);
  // mu_Cache[1].mu_Data.value(mu_AuxData);
  // mu_Cache[1].mu_Data.value(3);
  mu_AuxData.value(100);   //
  mu_MemData.value(10);


  // set carelist
  carelist.push_back("ExGntd");
  // carelist.push_back("State");
  // carelist.push_back("Data");
  carelist.push_back("AuxData");
  carelist.push_back("MemData");

}
char * Name(unsigned short r)
{
  if (r<=9999) return S0.Name(r-0);
  return NULL;
}
};
...
unsigned short StartStateManager::numstartstates = 10000;
...
bool mu__boolexpr45;
bool mu__boolexpr46;
  if (!((mu_ExGntd) == (mu_true))) mu__boolexpr46 = TRUE ;
  else {
...
bool mu__boolexpr55;
  ...
  mu__boolexpr55 = ((mu_Cache[mu_j].mu_State) == (mu_S)) ; 
}
