...
/****************************************
  The state queue, used in bfs algorithm.
 ****************************************/
class state_queue
{
protected:
...
/****************************************
  The state priority queue, used in local search algorithm.
 ****************************************/
#include<queue>

// makun: priority queue ordered by h value
struct cmp{
   bool operator()(state * a, state * b){
       return a->h > b->h;   // h value letter, priority higher 
   }
};

class state_priority_queue
{
protected:
  // state ** stateArray;		/* The actual array. */
...
public:
  ...
  inline void elts_clear(void) {
    while (num_elts > 0) {
      stateHeap.pop();
      num_elts--;
    } 
    cout << "  stateHeap is empty now!" << endl;
  }
...
/****************************************
  The state stack, used in DFS algorithm.
 ****************************************/
...
public:
  ...
  bool was_present_ls(state * &in, bool, bool);
  ...
  inline unsigned long NumElts_LS() {
