...
// makun
extern vector<pair<int, string>> attr_list;

bool is_attr(pair<int, string> const & p)
{
  bool flag = false;
  for (int i = 0; i < attr_list.size(); i++)
  {
    if (attr_list[i].first == p.first && attr_list[i].second == p.second)
    {
      flag = true;
    }
  }
...
public:
  ...
  virtual void print() {
    if (defined()){
      cout << name << ":" << value() << '\n';
      // TODO: code independent function
...
public:
  ...
  inline int goal_value(int val) {
    if (in_world)
      goalstate->setlong(byteOffset, val);
    else
      cvalue = val;
    return val;
  };
