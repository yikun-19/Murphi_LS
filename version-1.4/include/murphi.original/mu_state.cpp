...
:  max_active_states(mas), num_elts(0)
{
    // stateArray = new state *[max_active_states];
    // for (long i = 0; i < max_active_states; i++)	// Uli: avoid bzero
    //   stateArray[i] = NULL;
  
    priority_queue<state *, vector<state *>, cmp> stateHeap;
...
state_priority_queue::~state_priority_queue()
{
    ...
    swap(empty_pq, stateHeap);

}

int state_priority_queue::BytesForOneState(void)
{
#ifdef VER_PSEUDO
  // Pseudo ver: ptr + malloced state + approx. malloc&new overhead.
  return sizeof(state *) + sizeof(state) + 8;
...
void state_priority_queue::enqueue(state * e)
{
  if (num_elts < max_active_states) {
    // stateArray[rear] = e;
    // rear = (rear + 1) % max_active_states;
...
state *state_priority_queue::dequeue(void)
{
  state *retval;
  if (num_elts > 0) {
    // retval = stateArray[front];
...
state *state_priority_queue::top(void)
{
  if (num_elts > 0) {
    // return stateArray[front];
    return stateHeap.top();
  } else {
    Error.Notrace("Internal: Attempt to top() empty state queue.", "", "");
    return NULL;
  }
...
/****************************************
  class state_stack for searching the state space.
  ****************************************/
...
bool state_set::was_present_ls(state * &in, bool valid, bool permanent)
{
  // cout << "tabu device's size: " << num_elts <<endl;  // now: 
  if (num_elts < table_size) {
    ...
    table[num_collisions] = *in;
    in = &table[num_collisions];
    num_collisions = (num_collisions + 1) % table_size;
    num_elts ++;
    num_elts_reduced ++;
    return false;
  } 
  else if (num_elts == table_size) {  // num_sollisions point to the oldest state
    ...
    table[num_collisions] = *in;
    in = &table[num_collisions];
    num_elts_reduced ++;  // TODO: Duplicate state counted?
    num_collisions = (num_collisions + 1) % table_size;
    return false;
  }
  else {
    Error.Notrace("Cache Queue Error: Too many active states.");
  }
}
