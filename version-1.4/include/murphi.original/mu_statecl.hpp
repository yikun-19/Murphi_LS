...
/****************************************
  class state.
  ****************************************/
// Warning: DO NOT add member variables to this class unless you know exactly what you're doing.
// Since an instance of the state class is created for each table entry during initialization, adding stuff here will eat lots of memory.
...
{
public:
    BIT_BLOCK bits[BLOCKS_IN_WORLD]; // bit vector
    StatePtr previous;		   // state from which this state was reached.

// #ifdef LOCALSEARCH
    unsigned long h;     // makun: heuristic score for the state
    unsigned long d;     // makun: depth of the state
    // unsigned long doNo; // rule trace
// #endif
...
#define LOG2_UL 3
  ...
  inline bool operator ==(state & other) const {
      // int memcmp(const void *str1, const void *str2, size_t n) => compare the first n Bytes of str1 and str2
