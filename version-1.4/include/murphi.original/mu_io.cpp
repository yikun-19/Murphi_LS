...
#define VERIFY_LS_FLAG  "-localsearch"	/* makun: verify with our local search. */
...
void Error_handler::Notrace(const char *fmt ...)
{
  ...
  {
    if (StateSet_Ls != NULL) {	/* queue has been declared */
...
debug_sym(FALSE, "debug symmetry")
{
  ...
  if (main_alg.mode == argmain_alg::Verify_ls) {
    if (trace_file.value) {
      delete TraceFile;
      trace_file.reset(FALSE);
    }
...
void argclass::ProcessOptions(string_iterator * options)
{
  ...
  for (options->start(); !options->done(); options->next()) {
    ...
    if (strcmp(option, VERIFY_LS_FLAG) == 0) {
      cout << "You choose the local search option :-)" << endl;
      main_alg.set(argmain_alg::Verify_ls);
      continue;
    }
 ...
 argclass::PrintOptions(void)	// changes by Uli
{
  cout << "Options:\n"
      ...
      << "\t-localsearch  verify with our local search.\n"
...
//         << "\thash compression and breadth first search.\n";
  ...
  case argmain_alg::Verify_ls:
    cout << "\nAlgorithm:\n";
    cout << "\tVerification by our local search.\n";
    break;
...
void ReportManager::print_trace_with_theworld()	// changes by Uli
{
  ...
  if (args->main_alg.mode == argmain_alg::Verify_ls)
  {
    StateSet_Ls->print_trace_aux(curstate);
  }
  else
  {
...
void ReportManager::print_trace_with_curstate()	// changes by Uli
{
  ...
  if (args->main_alg.mode == argmain_alg::Verify_ls)
  {
    StateSet_Ls->print_trace(curstate);
  }
  else
  {
...
void ReportManager::print_progress_Ls(void)
{
  static bool initialized = FALSE;

  // pring progress report every <args->progress_count> new states found
  if (args->print_progress.value
      && StateSet_Ls->NumElts_LS() % args->progress_count.value == 0) {
    if (!initialized) {
      cout << "\nProgress Report:\n\n";
      initialized = TRUE;
    }
    cout << "\t"
	<< StateSet_Ls->NumElts_LS() << " states explored in "
	<< SecondsSinceStart() << "s, with "
	<< Rules->NumRulesFired() << " rules fired and "
	<< StateSet_Ls->QueueNumElts() << " states in the queue.\n";
    cout.flush();
  }
}
...
void ReportManager::print_retrack(void)
{
  ...
  if (args->main_alg.mode == argmain_alg::Verify_ls)
  {
    if (!StateSet_Ls->QueueIsEmpty()) {
    curstate = StateSet_Ls->QueueTop();
    StateCopy(workingstate, curstate);
    cout << "------------------------------\n";
    cout <<
