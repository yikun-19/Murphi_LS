...
//IM: incredible bug, if compiled with -O1: when this = NULL, in != NULL comes out to be true
ste *ste::reverse()
{
...
ste *symboltable::find(lexid * name) const
{
  ...
  {
    // std::cout << p->name->getname() << std::endl;
     p = p->next;
  }
  return (p);
}
...
{
  ste *p = find(name);
  // std::cout << "1111-" << this->getscopedepth() << std::endl;
  if (p == NULL) {
...
ste *symboltable::mk_lookup(const char* mk_name)
{
  ste *p = mk_topste();
  // std::cout << " ------ finding... " << mk_name << std::endl;
  while (p != NULL && strcmp(p->name->getname(), mk_name) != 0)
  {
    // std::cout << p->name->getname() << std::endl;
    p = p->next;
  }
  if (p == NULL) {
    // Error.Error("%s undeclared.", mk_name);
