...
public:
  ...
  char *getstring() 
  {
    char *expr_str = "idle string";
    // std::cout << "value: " << type << std::endl;
    // sprintf(expr_str, "%d",  value);
    return expr_str;
  }
  ...
  virtual const char *generate_inv_str();
  virtual const char *get_inv_items(std::vector<std::string> &vec);
};
...
myceil,
  ...
  virtual const char *generate_code();
  virtual const char *generate_inv_str();
  void generate_inv_str(std::vector<std::string> vec);
};
...
public:
  ...
  {
    char *inv_str_res = "";
    char *str_1 = left->getstring();
    char *str_2 = right->getstring();
    // sprintf(inv_str, "%d",  value);
    
    sprintf(inv_str_res, "%s%d%s", str_1, op, str_2);
    return inv_str_res;
  }
...
struct exprlist {
  ...
  // code generation
      ...
      virtual const char *generate_inv_str(const char *name, ste * formals);
};
