...
bool mk_judge_string(std::vector<std::string> &vec, std::string &s)
{
  if (s == "mu_true" || s == "mu_false") return true;
...
bool mk_judge_string_rule(std::vector<std::string> &vec, std::string &s)
{
  if (s == "mu_i" || s == "mu_j") return true;
  for (int i = 0; i < vec.size(); i++)
  {
    if (vec[i] == s) return true;
  }
...
const char *expr::generate_inv_str()
{
  if (constval)
    if (type_equal(type, realtype))
      return tsprintf("%le", rvalue);	// AP: value of a real constant
    ...
    Error.Error("Internal: a basic expression that wasn't a constant called expr.");
    return "ERROR!";
  }
}

const char *expr::get_inv_items(std::vector<std::string> &vec)
{
  std::string s;
  ...
  {
    if (type_equal(type, realtype))
      s = tsprintf("%le", rvalue);	// AP: value of a real constant
    else
      s = tsprintf("%d", value);
    if (!mk_judge_string(vec, s)) vec.emplace_back(s);
    return s.c_str();
  }
  else {
    Error.Error("get_int_items Internal: a basic expression that wasn't a constant called expr.");
    return "ERROR!";
  }
}
...
//IM: for math functions
const char *mathexpr::generate_inv_str()
{
  if (constval)
    return tsprintf("%le", rvalue);
  switch (getfuntype()) {
  case mylog:
	...
	return tsprintf("log((double)%s)", arg1->generate_inv_str());
      break;
    }
  case mylog10:
    {
	...
	return tsprintf("log10((double)%s)", arg1->generate_inv_str());
      break;
    }
  case myexp:
    {
	...
	return tsprintf("exp((double)%s)", arg1->generate_inv_str());
      break;
    }
  case mysin:
    {
	...
	return tsprintf("sin((double)%s)", arg1->generate_inv_str());
      break;
    }
  case mycos:
    {
	...
	return tsprintf("cos((double)%s)", arg1->generate_inv_str());
      break;
    }
  case mytan:
    {
	...
	return tsprintf("tan((double)%s)", arg1->generate_inv_str());
      break;
    }
  case myfabs:
    {
	...
	return tsprintf("fabs((double)%s)", arg1->generate_inv_str());
      break;
    }
  case myfloor:
    {
	...
	return tsprintf("floor((double)%s)", arg1->generate_inv_str());
      break;
    }
  case myceil:
    {
	...
	return tsprintf("ceil((double)%s)", arg1->generate_inv_str());
      break;
    }
  case mysqrt:
    {
	...
	return tsprintf("sqrt((double)%s)", arg1->generate_inv_str());
      break;
    }
  case myfmod:
    {
	...
	return tsprintf("fmod((double)%s,(double)%s)",
			arg1->generate_code(), arg2->generate_inv_str());
      break;
    }
  case mypow:
    {
  ...
  case myasin:
	...
	return tsprintf("asin((double)%s)", arg1->generate_inv_str());
      break;
    }
  case myacos:
    {
	...
	return tsprintf("acos((double)%s)", arg1->generate_inv_str());
      break;
    }
  case myatan:
    {
	...
	return tsprintf("atan((double)%s)", arg1->generate_inv_str());
      break;
    }
  case mysinh:
    {
	...
	return tsprintf("sinh((double)%s)", arg1->generate_inv_str());
      break;
    }
  case mycosh:
    {
	...
	return tsprintf("cosh((double)%s)", arg1->generate_inv_str());
      break;
    }
  case mytanh:
    {
	...
	return tsprintf("tanh((double)%s)", arg1->generate_inv_str());
      break;
    }
  }
  return NULL;
...
/********************
 makun: code for boolexpr
 ********************/
const char *boolexpr::generate_inv_str()
{
  if (constval)
    return tsprintf("%d", value);
  else 
  {
    ...
    {
    case IMPLIES:
      // fprintf(codefile, " ( %s ) => (%s) ", left->generate_inv_str(), right->generate_inv_str());
      return tsprintf("(=> %s %s)", left->generate_inv_str(), right->generate_inv_str());
      break;
    case '|':
      // fprintf(codefile, " ( %s ) | (%s) ", left->generate_inv_str(), right->generate_inv_str());
      return tsprintf("(or %s %s)", left->generate_inv_str(), right->generate_inv_str());
      break;
    case '&':
      // fprintf(codefile, " ( %s ) & (%s) ", left->generate_inv_str(), right->generate_inv_str());
      return tsprintf("(and %s %s)", left->generate_inv_str(), right->generate_inv_str());
      break;
    default:
      Error.Error("Internal: funky value for op in boolexpr::generate_inv_str()");
      return "ERROR!";
      break;
    }
  }
...
/********************
 makun: code for boolexpr
 ********************/
const char * boolexpr::get_inv_items(std::vector<std::string> &vec)
{
  // std::cout << "debug-bool" << std::endl;
  std::string s;
  if (constval)
    {
      s = tsprintf("%d", value);
  ...
  else 
  {
    ...
    case IMPLIES:
      // fprintf(codefile, " ( %s ) => (%s) ", left->generate_inv_str(), right->generate_inv_str());
      left->get_inv_items(vec);
      right->get_inv_items(vec);
      return tsprintf("(=> %s %s)", left->generate_inv_str(), right->generate_inv_str());
      break;
    case '|':
      left->get_inv_items(vec);
...
const char *notexpr::generate_inv_str()
{
  return tsprintf("(not %s)", left->generate_inv_str());
}

const char *notexpr::get_inv_items(std::vector<std::string> &vec)
{
...
const char *equalexpr::generate_inv_str()
{
  if (constval)
    return tsprintf("%d", value);
  else {
    ...
    case '=':
      return tsprintf("(= %s %s)",
		      left->generate_inv_str(), right->generate_inv_str());
      break;
    case NEQ:
      return tsprintf("(not (= %s %s))",
		      left->generate_inv_str(), right->generate_inv_str());
      break;
    default:
      Error.Error
	  ("Internal: exciting value for op in equalexpr::generate_inv_str().");
      return "ERROR!";
      break;
    }
  }
...
const char *equalexpr::get_inv_items(std::vector<std::string> &vec)
{
  // std::cout << "debug-equ" << std::endl;
  std::string s;
  if (constval)
  {
  ...
  else 
  {
    ...
    case NEQ:
    case '=':
      s = left->generate_inv_str();
      if (!mk_judge_string(vec, s)) vec.emplace_back(s); 
      s = right->generate_inv_str();
      if (!mk_judge_string(vec, s)) vec.emplace_back(s);
      left->get_inv_items(vec);
      right->get_inv_items(vec);
      break;
    default:
      Error.Error("Internal: funky value for op");
      break;
    }
  }
  return "equexpr";
...
const char *compexpr::generate_inv_str()
{
  if (constval)
    return tsprintf("%d", value);
  else {
    ...
    case GEQ:
      return tsprintf("(%s) >= (%s)", left->generate_inv_str(), right->generate_inv_str());
    default:
      Error.Error("Internal: odd value in compexpr");
      return "ERROR!";
    }
  }
}
...
const char *arithexpr::generate_inv_str()
{
  if (constval)
    if (type_equal(type, realtype))
      return tsprintf("%le", rvalue);	// AP: value of a real arithmetic expression (+,-)
...
const char *unexpr::generate_inv_str()
{
  if (constval)
    if (type_equal(type, realtype))
      return tsprintf("%le", rvalue);	// AP: value of a real unary expression
    ...
    switch (op) {
    case '+':
      return left->generate_inv_str();
    case '-':
      return tsprintf(" - (%s)", left->generate_inv_str());
    default:
      Error.Error("Internal: bad operator in arithexpr");
      return "ERROR!";
    }
...
const char *mulexpr::generate_inv_str()
{
  if (constval)
    if (type_equal(type, realtype))
      return tsprintf("%le", rvalue);	// AP: value of a real arithmetic expression (*,/)
...
const char *condexpr::generate_inv_str()
{
  return tsprintf("(%s) ? (%s) : (%s)",
		  test->generate_inv_str(),
		  left->generate_inv_str(), right->generate_inv_str());
}
...
void make_quant_fors(ste * quants, bool flag = true)
{
  if (quants != NULL && quants->getvalue()->getclass() == decl::Quant) {
    make_quant_fors(quants->getnext(), flag);
    if(flag) ((quantdecl *) quants->getvalue())->make_for();
  }
}


void make_quant_closes(ste * quants, bool flag = true)
{
  if (quants != NULL && quants->getvalue()->getclass() == decl::Quant) {
    make_quant_closes(quants->getnext(), flag);
    if(flag) fprintf(codefile, "};\n");
  }
}
...
const char *quantexpr::generate_inv_str()
{
  int num = new_int();
  char *temp = tsprintf("mu__quant%d", num);    // TODO: forall expr
  bool isforall = ((op == FORALL) ? TRUE : FALSE);

  // forall: tmp = true, if (!left) tmp = false; 
  // not forall: tmp = false; if (left) tmp = true;

  // Uli: initialization in the declaration not allowed in this case
  // fprintf(codefile, "bool %s; \n%s = %s;\n", temp, temp, (isforall ? "TRUE" : "FALSE"));
  // fprintf(codefile, "{\n");

  make_quant_fors(parameter, false); 

  /* so the new parameter doesn\'t conflict with others if it\'s reused. */
  // fprintf(codefile, "if ( %s(%s) )\n", (isforall ? "!" : ""), left->generate_inv_str());

  char *s_tmp = tsprintf("%s", left->generate_inv_str());  

  // Uli: a "goto" to exit the for loop seemed to cause problems on some
  //      compilers when there were local variables
  // fprintf(codefile, "  { %s = %s; break; }\n", temp, (isforall ? "FALSE" : "TRUE"));
  
  make_quant_closes(parameter, false);
...
const char *quantexpr::get_inv_items(std::vector<std::string> &vec)
{
...
const char *designator::generate_inv_str()
{
  switch (dclass) {
  case Base:
    return origin->getvalue()->generate_code();
  case ArrayRef:
    return tsprintf("%sppp0%s0qqq",    // makun: not beautiful, but avoid use of '[' and ']' in smt-lib format string
		    left->generate_inv_str(), arrayref->generate_inv_str());
    break;
  case FieldRef:
    return tsprintf("%s.%s",
		    left->generate_inv_str(), fieldref->getvalue()->mu_name);
    break;
  default:
    Error.Error
	("Internal: Strange and mysterious values for designator::dclass.");
...
/********************
 makun: code for boolexpr
 ********************/
const char *designator::get_inv_items(std::vector<std::string> &vec)
{
  // std::cout << "debug-designator" << std::endl;
  std::string s;
  switch (dclass) {
  case Base:
    s = origin->getvalue()->generate_code();
    if (!mk_judge_string(vec, s)) vec.emplace_back(s);
    return s.c_str();
    break;
  case ArrayRef:
    s = tsprintf("%sppp0%s0qqq",    // makun: not beautiful, but avoid use of '[' and ']' in smt-lib format string
		    left->generate_inv_str(), arrayref->generate_inv_str());
    if (!mk_judge_string(vec, s)) vec.emplace_back(s);
    return s.c_str();
    break;
...
const char *exprlist::generate_inv_str(const char *name, ste * formals)
{
  exprlist *ex = this;
  if (this == NULL) {
    // exprlist_buffer_end = exprlist_buffer;
	...
	if (f->gettype() != ex->e->gettype() &&
	  ...
	  sprintf(exprlist_buffer_end, ", %s", ex->e->generate_inv_str());
      } else {
	sprintf(exprlist_buffer_end, ", %s_undefined_var",
		f->gettype()->generate_code());
      }
...
const char *funccall::generate_inv_str()
{
  funcdecl *f = (funcdecl *) func->getvalue();
  return tsprintf("%s( %s )",
		  //im: for imported functions, "mu_" does not have to be prefixed to the function name
		  ...
		  func->getvalue()->generate_code(),
		  actuals !=
		  NULL ? actuals->generate_inv_str(func->getname()->getname(),
						f->params) : "");
}
...
const char *isundefined::generate_inv_str()
{
  return tsprintf("%s.isundefined()", left->generate_inv_str());
}
...
const char *ismember::generate_inv_str()
{
  return tsprintf("(%s>=%d && %s<=%d)",
		  left->generate_inv_str(), t->getleft(),
		  left->generate_inv_str(), t->getright());
}
...
const char *multisetcount::generate_inv_str()
{
  /* set->gettype()->getelementtype()->generate_code(), /* element type */
  /* set->gettype()->generate_code(), /* multiset type */
  /*  multisetcount_num,               /* procedure number */
  ...
  fprintf(codefile, "  {\n" "  %s_id %s;\n" "  for (%s = 0; ; %s=%s+1)\n" "    {\n" "      if (%s.valid[(int)%s].value())\n"	// Uli 10-98
	  ...
	  index->getvalue()->generate_code(),	/* index name */
	  index->getvalue()->generate_code(),	/* index name */
	  set->generate_inv_str(),	/* multiset variable name */
	  index->getvalue()->generate_code()	/* index name */
      );
  fprintf(codefile, "	  if ( %s ) %s++;\n" "	}\n" "      if (%s == %d-1) break;\n" "    }\n" "  }\n", filter->generate_inv_str(),	/* bool expression */
	  temp, index->getvalue()->generate_code(),	/* index name */
	  ((multisettypedecl *) set->gettype())->getindexsize()	/* max size */
      );
  fprintf(codefile, "/*** end multisetcount %d declaration ***/\n",
...
const char *stmt::get_items(std::vector<std::string> &vec)
{
  std::string s;
  s = "";
  return "ERROR!";
...
const char *assignment::get_items(std::vector<std::string> &vec)
{
  std::string s;
  if (src->isdesignator() && src->gettype()->issimple() && src->checkundefined())
  {
    s = tsprintf("%s", target->generate_inv_str());
    if (!mk_judge_string(vec, s)) vec.emplace_back(s);
  } else {
    s = tsprintf("%s", target->generate_inv_str());
    if (!mk_judge_string(vec, s)) vec.emplace_back(s);
...
const char *ifstmt::get_items(std::vector<std::string> &vec)
{
  std::string s;
...
const char *caselist::get_items(std::vector<std::string> &vec)
{
  std::string s;
  for (stmt * b = body; b != NULL; b = b->next)
    b->get_items(vec);
...
const char *switchstmt::get_items(std::vector<std::string> &vec)
{
  std::string s;
  /* The explicit cast seems to be necessary to allow things like
   * switch arr[i]... */
...
const char *forstmt::get_items(std::vector<std::string> &vec)
{
  std::string s;
  for (stmt * b = body; b != NULL; b = b->next)
    b->get_items(vec);
 ...
 ********************/
static void
generate_rules_aux(int quantindex, char *namequants_end, char *quantactuals_end, simplerule * therule)
{
  rulerec *rr;
  ...
  if (quantindex >= numquants) {
    /* generate the code for the rule and enroll it into appropriate list */
    ...
    rr = new rulerec(tsprintf("%s%s", therule->name, namequants), condname, rulename);
    ...
    case rule::Startstate:	/* This function seems not to be called on start states! */
      ...
      break;
    case rule::Invar:
      rr->next = theprog->invariantlist;  // linked list
...
const char *ste::generate_decls()
{
  if (this != NULL) {
    if (next != NULL && next->scope == scope)
      next->generate_decls();
    value->generate_decl();
    // std::string s = value->generate_code();  // makun
    // std::cout << "ste: " << s << std::endl;  // this->name->getname()
    // std::cout << "------it: " << symtab->mk_lookup(s.replace(0, 3, "").c_str())->value->gettype()->gettype() << std::endl;
  }
  return "ERROR!";
}
...
/********************
  get_inv_str()
  -- called by traversal_rules()
  --
  -- get invariants expression string and store it in yylval.string
********************/
// generate_rules_aux(int quantindex, char *namequants_end, char *quantactuals_end, simplerule * therule)
// static void get_inv_str_aux(int quantindex, char *namequants_end, char *quantactuals_end, simplerule * therule)
// {   
//     char *condname;
//     char *rulename;
...
//         if (therule->getclass() == rule::Invar)
//         {
//             rr->next = theprog->invariantlist;  // linked list
//             // char test_str[4] = {'t', 'e', 's', 't'};
//             // std::cout << "11111" << test_str << std::endl;
//             // cout << "11111" << therule->condition->getstring() << endl;
//             // yylval.string = therule->condition->getstring();
//             yylval.string = test_str;
//             theprog->invariantlist = rr;
//         }
//     }
// }
...
// makun: Try
// static void get_inv_str(ste * enclosures, simplerule * therule)
// {
//     int i = 0;
...
//     // generate rules by calling to main rule with quantifier value
//     get_inv_str_aux(0, namequants, quantactuals, therule);
// }


// makun: ppp0 qqq0 => [ ]
std::string mk_trans_bracket(std::string src_str, bool flag = false)
{
  std::string::size_type pos = 0, pos_aux = 0;
  while((pos = src_str.find(std::string("ppp0"))) != std::string::npos)   //替换所有指定子串
  {
    src_str.replace(pos, 4, "[");
  }
  pos = 0;
  while((pos = src_str.find(std::string("0qqq"))) != std::string::npos)   //替换所有指定子串
  {
    src_str.replace(pos, 4, "]");
  }
  
  pos = 0;
  while((pos = src_str.find(std::string("mu_i"))) != std::string::npos && ((pos >= 1 && src_str.at(pos-1) != '.') || pos == 0))   //替换所有指定子串
  {
    src_str.replace(pos, 4, "1");
  }
  pos = 0;
  while((pos = src_str.find(std::string("mu_j"))) != std::string::npos && ((pos >= 1 && src_str.at(pos-1) != '.') || pos == 0))   //替换所有指定子串
  {
    src_str.replace(pos, 4, "2");
  }
  pos = 0;
  while((pos = src_str.find(std::string("mu_n1"))) != std::string::npos)   //替换所有指定子串
  {
    src_str.replace(pos, 5, "1");
  }
  pos = 0;
  while((pos = src_str.find(std::string("mu_n2"))) != std::string::npos)   //替换所有指定子串
  {
    src_str.replace(pos, 5, "2");
  }
  pos = 0;
  while((pos = src_str.find(std::string("mu_addr"))) != std::string::npos && ((pos >= 1 && src_str.at(pos-1) != '.') || pos == 0))   //替换所有指定子串
  {
    src_str.replace(pos, 7, "1");
  }
  pos = 0;
  while((pos = src_str.find(std::string("mu_h"))) != std::string::npos && ((pos >= 1 && src_str.at(pos-1) != '.') || pos == 0))   //替换所有指定子串
  {
    src_str.replace(pos, 4, "1");
  }
  pos = 0;
  while((pos = src_str.find(std::string("mu_l"))) != std::string::npos && ((pos >= 1 && src_str.at(pos-1) != '.') || pos == 0))   //替换所有指定子串
  {
    src_str.replace(pos, 4, "4");
  }
  pos = 0;
  while((pos = src_str.find(std::string("mu_a"))) != std::string::npos && ((pos >= 1 && src_str.at(pos-1) != '.') || pos == 0))   //替换所有指定子串
  {
    if ((pos_aux = src_str.find(std::string("firstRead"))) != std::string::npos || (pos_aux = src_str.find(std::string("memory"))) != std::string::npos)
    {
      src_str.replace(pos, 4, "1");
    }
    else{
  ...
  {
    pos = 0;
    while((pos = src_str.find(std::string("mu_"))) != std::string::npos)
    {
      src_str.replace(pos, 3, "");
    }
  }
  return src_str;
}

// makun: aux function for generating goalstate automatically
z3::expr mk_get_expr(z3::context& c, Z3_func_decl& constant)   // get expression from constants of enum_sort
{
   z3::func_decl temp = z3::to_func_decl(c, constant);
   return z3::to_expr(c, Z3_mk_app(c, temp, 0, 0));
}

z3::sort mk_create_enum(z3::context &c, Z3_symbol sort_name, std::vector<std::string>& enum_values, const int& count, Z3_func_decl * consts, Z3_func_decl * testers)
{
    
    Z3_symbol names[count];
    for (int i = 0; i < count; i++)
    {
        Z3_symbol tmp_symbol = Z3_mk_string_symbol(c, enum_values[i].c_str());
        names[i] = tmp_symbol;
    }
    
    Z3_sort _s = Z3_mk_enumeration_sort(c, sort_name, count, names, consts, testers);
    z3::sort s = z3::to_sort(c, _s);
...
int generate_startstates()
{
  ...
  // std::cout << "=================== running z3-solver ==============================" << std::endl;
  fprintf(codefile, "void Goal_value() {  // makun: set goalstate\n");
  fprintf(codefile, "    if (goal_value_flag) return;\n");
  fprintf(codefile, "    goal_value_flag = true;\n\n");

  z3::context z3_context;

  // get enum context from decl part in Murphi
  std::vector<std::string> enum_sort1_values;
  ste *decl_str;
  for (decl_str = theprog->globals; decl_str != NULL; decl_str = decl_str->getnext())
  {
    if (decl_str->getvalue()->getclass() == typedecl::Enum && decl_str->getvalue()->gettype()->generate_code() != NULL) // all of them are constdecl, not enumdecl
    {
      // fprintf(codefile, "    %s : %s\n", decl_str->getvalue()->generate_code(), decl_str->getvalue()->gettype()->generate_code());
      enum_sort1_values.emplace_back(decl_str->getvalue()->generate_code());  // TODO: distinguish different enum types
    }
  }


  const int enum_sort1_count = enum_sort1_values.size();

  Z3_symbol enum_name = Z3_mk_string_symbol(z3_context, "mu_enum_sort");
  Z3_func_decl enum_consts[enum_sort1_count];
  Z3_func_decl enum_testers[enum_sort1_count];
  z3::sort mu_enum_sort = mk_create_enum(z3_context, enum_name, enum_sort1_values, enum_sort1_count, enum_consts, enum_testers);
  const int sort_num = 1;
  Z3_symbol const con_sort_names[sort_num] = {mu_enum_sort.name()};
  Z3_sort const con_sorts[sort_num] = {mu_enum_sort};


  simplerule *sr_inv;
  std::vector<std::string> inv_items_set;
  std::vector<std::string> rule_condition_items_set;
  std::vector<std::string> rule_stmt_items_set;
  for (sr_inv = simplerule::SimpleRuleList; sr_inv != NULL; sr_inv = sr_inv->NextSimpleRule) 
  {
    // get items from invariant
    if (sr_inv->getclass() == rule::Invar && sr_inv != error_rule) 
    {
      sr_inv->condition->get_inv_items(inv_items_set);
    }

    // get items from rulebase
    else if (sr_inv->getclass() == rule::Simple && sr_inv != error_rule) 
    {
      rule_stmt_items_set.clear();
      
      stmt * sr_stmt;
      for(sr_stmt = sr_inv->body; sr_stmt != NULL; sr_stmt = sr_stmt->next)
      {
        sr_stmt->get_items(rule_stmt_items_set);
      }
      
      for (int i = 0; i < rule_stmt_items_set.size(); i++)
      {
        if (!mk_judge_string_rule(enum_sort1_values, rule_stmt_items_set[i]) && mk_judge_string_rule(inv_items_set, rule_stmt_items_set[i]))
        {
          std::cout << sr_inv->name << std::endl;
          sr_inv->condition->get_inv_items(rule_condition_items_set);   
        }
      }
    }
  }

  // remove inv items from rule items
  for (auto it = rule_condition_items_set.begin(); it != rule_condition_items_set.end(); )
  {
    if (mk_judge_string_rule(inv_items_set, *it))
    {
      it = rule_condition_items_set.erase(it);
    }
    else
      it++;
  }

  // remove enum const from invariant
  for (int i = 0; i < enum_sort1_values.size(); i++)
  {
    auto it = find(inv_items_set.begin(), inv_items_set.end(), enum_sort1_values[i]);
    if ( it != inv_items_set.end())
    {
      inv_items_set.erase(it);
    }
  }

  // for (int i = 0; i < inv_items_set.size(); i++)
  // {
  //   std::cout << inv_items_set[i] << std::endl;
  // }

  const int inv_values_num = inv_items_set.size();
  Z3_symbol con_decl_names[inv_values_num];
  Z3_func_decl con_decls[inv_values_num];
  int con_decl_names_ptr = 0, con_decls_ptr = 0;
  std::vector<z3::expr> attr_list;

  // for (int i = 0; i < inv_items_set.size(); i++)
  // { 
  //   // bool flag = (symtab->mk_lookup(inv_items_set[i].replace(0, 3, "").c_str())->value->gettype()->gettype());
  //   bool flag = true;
  //   std::cout << inv_items_set[i] << " : " << flag << std::endl;
  // }

  std::cout << std::endl << "-------begin: declare part!-------" << std::endl;
  for (int i = 0; i < inv_items_set.size(); i++)  // TODO: 
  { 
    // TODO: int_const, not equal 0
    if (inv_items_set[i] == std::string("mu_i") || inv_items_set[i] == std::string("mu_j") \ 
        || inv_items_set[i] == std::string("mu_MemData") || inv_items_set[i] == std::string("mu_AuxData") \
        || inv_items_set[i] == std::string("mu_Cacheppp0mu_i0qqq.mu_Data") \
        || inv_items_set[i] == std::string("mu_n1") || inv_items_set[i] == std::string("mu_n2") || inv_items_set[i] == std::string("mu_addr") \ 
        || inv_items_set[i] == std::string("mu_h") || inv_items_set[i] == std::string("mu_l") || inv_items_set[i] == std::string("mu_a") \ 
        || inv_items_set[i] == std::string("mu_nodeppp0mu_i0qqq.mu_cacheppp0mu_j0qqq.mu_addr") || inv_items_set[i] == std::string("mu_nodeppp0mu_i0qqq.mu_cacheppp0mu_j0qqq.mu_data") || inv_items_set[i] == std::string("mu_memoryppp0mu_a0qqq.mu_data") \
       ) 
    {
      con_decl_names[con_decl_names_ptr++] = Z3_mk_string_symbol(z3_context, inv_items_set[i].c_str());
      z3::expr mu_tmp_decl = z3_context.int_const(inv_items_set[i].c_str());
      attr_list.push_back(mu_tmp_decl);
      con_decls[con_decls_ptr++] = mu_tmp_decl.decl();
      std::cout << "  " << inv_items_set[i] << " is declared as int type" << std::endl;
    }
    // enum type const
    else if (inv_items_set[i] == std::string("mu_nppp0mu_i0qqq") || inv_items_set[i] == std::string("mu_nppp0mu_j0qqq") \
              || inv_items_set[i] == std::string("mu_Cacheppp0mu_i0qqq.mu_State") || inv_items_set[i] == std::string("mu_Cacheppp0mu_j0qqq.mu_State") \
              || inv_items_set[i] == std::string("mu_Sta.mu_Procppp0mu_i0qqq.mu_CacheState") || inv_items_set[i] == std::string("mu_Sta.mu_Procppp0mu_j0qqq.mu_CacheState") \
              || inv_items_set[i] == std::string("mu_Procsppp0mu_n10qqq.mu_Cacheppp0mu_h0qqqppp0mu_a0qqq.mu_State") || inv_items_set[i] == std::string("mu_Procsppp0mu_n20qqq.mu_Cacheppp0mu_h0qqqppp0mu_a0qqq.mu_State") \
              || inv_items_set[i] == std::string("mu_LockOwnerppp0mu_h0qqqppp0mu_l0qqq.mu_State") || inv_items_set[i] == std::string("mu_Homesppp0mu_h0qqq.mu_LDirppp0mu_l0qqq.mu_State") \
              || inv_items_set[i] == std::string("mu_nodeppp0mu_n10qqq.mu_cacheppp0mu_addr0qqq.mu_state") || inv_items_set[i] == std::string("mu_nodeppp0mu_n20qqq.mu_cacheppp0mu_addr0qqq.mu_state") \
              || inv_items_set[i] == std::string("mu_nodeppp0mu_i0qqq.mu_cacheppp0mu_j0qqq.mu_state") || inv_items_set[i] == std::string("mu_replace") \
              || inv_items_set[i] == std::string("mu_Lnppp010qqq.mu_cache.mu_lineppp010qqq.mu_state") || inv_items_set[i] == std::string("mu_Lnppp020qqq.mu_cache.mu_lineppp010qqq.mu_state") || inv_items_set[i] == std::string("mu_Lnppp030qqq.mu_cache.mu_lineppp010qqq.mu_state") \
              || inv_items_set[i] == std::string("mu_L1ppp040qqq.mu_cache.mu_lineppp010qqq.mu_state") || inv_items_set[i] == std::string("mu_L1ppp050qqq.mu_cache.mu_lineppp010qqq.mu_state") || inv_items_set[i] == std::string("mu_L1ppp060qqq.mu_cache.mu_lineppp010qqq.mu_state") || inv_items_set[i] == std::string("mu_L1ppp070qqq.mu_cache.mu_lineppp010qqq.mu_state") \
            ) // another op in enum decl
    {
      con_decl_names[con_decl_names_ptr++] = Z3_mk_string_symbol(z3_context, inv_items_set[i].c_str());
      z3::expr mu_tmp_decl = z3_context.constant(inv_items_set[i].c_str(), mu_enum_sort);
      attr_list.push_back(mu_tmp_decl);
      con_decls[con_decls_ptr++] = mu_tmp_decl.decl();
      std::cout << "  " << inv_items_set[i] << " is declared as enum type" << std::endl;
    }
    // bool type
    // else if (symtab->mk_lookup(inv_items_set[i].replace(0, 3, "").c_str())->value->gettype()->gettype() == booltype)
    else if (symtab->mk_lookup(inv_items_set[i].replace(0, 3, "").c_str())->value->gettype()->gettype() == booltype)
    {
      con_decl_names[con_decl_names_ptr++] = Z3_mk_string_symbol(z3_context, ("mu_" + inv_items_set[i]).c_str());
      z3::expr mu_tmp_decl = z3_context.bool_const(("mu_" + inv_items_set[i]).c_str());
      attr_list.push_back(mu_tmp_decl);
      con_decls[con_decls_ptr++] = mu_tmp_decl.decl();
      std::cout << "  " << inv_items_set[i] << " is declared as bool type" << std::endl;
    }
    else
    {
      std::cout << "  " << inv_items_set[i] << " is not declared" << std::endl;
    }
  }
  std::cout << "-------end: declare part!---------" << std::endl << std::endl ;


  z3::solver s(z3_context);

  // transform inv to conjecture of solver s
  for (sr_inv = simplerule::SimpleRuleList; sr_inv != NULL; sr_inv = sr_inv->NextSimpleRule) 
  {
    if (sr_inv->getclass() == rule::Invar && sr_inv != error_rule) 
    {
      std::string z3_input_inv_string = std::string(sr_inv->condition->generate_inv_str());
      std::string::size_type pos = 0;
      while((pos = z3_input_inv_string.find(std::string("mu_true"))) != std::string::npos)   //替换所有指定子串
      {
        z3_input_inv_string.replace(pos, 7, "true");
      }
      while((pos = z3_input_inv_string.find(std::string("mu_false"))) != std::string::npos)   //替换所有指定子串
      {
        z3_input_inv_string.replace(pos, 8, "false");
      }
      std::string z3_inv_str = std::string("(assert ") + z3_input_inv_string + std::string(")");
      std::cout << "invariant in string format: " << std::endl << "  " << z3_inv_str << std::endl << std::endl;

      try{
        std::cout << "--transfering input string to Z3 format... " << std::endl;
        Z3_ast_vector z3_inv_str_ast = Z3_parse_smtlib2_string(z3_context, z3_inv_str.c_str(), sort_num, con_sort_names, con_sorts, inv_values_num, con_decl_names, con_decls);
        z3::expr_vector conjecture_tmp(z3_context, z3_inv_str_ast);
        std::cout << "--transfer successfully! " << std::endl << std::endl;
        s.add(!conjecture_tmp.back());

        // TODO: data property 
        // std::string z3_data_constriant = std::string("(assert ") + std::string("(and (> mu_MemData 1) (> mu_AuxData 1))") + std::string(")");
        // Z3_ast_vector z3_data_constriant_ast = Z3_parse_smtlib2_string(z3_context, z3_data_constriant.c_str(), sort_num, con_sort_names, con_sorts, inv_values_num, con_decl_names, con_decls);
        // z3::expr_vector conjecture_data_tmp(z3_context, z3_data_constriant_ast);
        // s.add(conjecture_data_tmp.back());

      }catch (...){
        std::cout << "Z3's input is not valid!" << std::endl;
      }
    }
  }

  // get result from solver s 
  if (s.check() == z3::unsat) {
    std::cout << "The invariant is obviously broken!" << std::endl; 
  }
  else if(s.check() == z3::unknown){
    std::cout << "Z3 do not know the answer." << std::endl; 
  }
  else {
    std::cout << "Z3 give a goal state successfully!" << std::endl << std::endl;
    
    // fprintf(codefile, "    // static mu_1_client mu_i;\n"); 
    // fprintf(codefile, "    // r-value is int type: mu_i.goal_value(1);\n");
    // fprintf(codefile, "    // r-value is other type: mu_n[1].goal_value(mu_E);\n");
    fprintf(codefile, "\n    // set goalstate value: \n");
        
    z3::model m = s.get_model();
    for (int i = 0 ; i < attr_list.size(); i++)
    {
      if (mk_trans_bracket(attr_list[i].to_string()) != "1" && mk_trans_bracket(attr_list[i].to_string()) != "2" && mk_trans_bracket(attr_list[i].to_string()) != "4")
      {
        // std::cout << attr_list[i] << " : " << m.eval(attr_list[i]) << std::endl;
        fprintf(codefile, "    %s.value(%s);\n", mk_trans_bracket(attr_list[i].to_string()).c_str(), m.eval(attr_list[i]).to_string().c_str());
      }
    }
  ...
  fprintf(codefile, "    carelist.clear();  \n");
  for (int i = 0 ; i < attr_list.size(); i++)
  {
    if (mk_trans_bracket(attr_list[i].to_string()) != "1" && mk_trans_bracket(attr_list[i].to_string()) != "2")
      fprintf(codefile, "    carelist.push_back(\"%s\");\n", mk_trans_bracket(attr_list[i].to_string(), true).c_str()); // TODO: change
  }
  
  fprintf(codefile, "\n    // set carelist about rulebase \n");
  fprintf(codefile, "    carelist_rb.clear();  \n");
  for (int i = 0 ; i < rule_condition_items_set.size(); i++)
  {
    if (rule_condition_items_set[i] != " ")
      fprintf(codefile, "    carelist_rb.push_back(\"%s\");\n", mk_trans_bracket(rule_condition_items_set[i], true).c_str());
  }

  fprintf(codefile, "}\n");

  fprintf(codefile, "void Goal_value1() {  // makun: set goalstate\n");
  fprintf(codefile, "    if (goal_value_flag1) return;\n");
  fprintf(codefile, "    goal_value_flag1 = true;\n}\n\n");

  fprintf(codefile, "void FlowSetter() {  // makun: set a flow to improve carelist\n");
  fprintf(codefile, "    vector<int> flow1 = {20, 16, 8, 4, 22};\n");
  fprintf(codefile, "    flows.push_back(flow1);\n}\n\n");
