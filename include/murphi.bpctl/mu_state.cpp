...
{
  // stateArray = new state *[max_active_states];
  priority_queue<state *, vector<state *>, greater<int>> stateArray;
...
int
 state_priority_queue::BytesForOneState(void)
{
...
void state_priority_queue::enqueue(state * e)
{
  ...
  //   Error.Notrace("Internal Error: Too many active states.");
  // }
  if (stateArray.size() < max_active_states)
  {
      stateArray.push(e);
  }
  else
