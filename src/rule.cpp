...
/********************
  ...
  ********************/
int simplerulenum = 0;
simplerule::simplerule(ste * enclosures, expr * condition, ste * locals, stmt * body, int priority)
:rule(), name(name), enclosures(enclosures), condition(condition),
locals(locals), body(body), condname(NULL), rulename(NULL), priority(priority)
{
...
:  simplerule(enclosures, test, NULL, NULL, 0)
{
  ...
  // this->inv_str = test->getstring(); repalced by invariantlist = r->condition->getstring()
